...
class Node:
  ...
  def __init__(self, data: Any = None, prev: Node = None, next: Node = None) -> None:
    self.data = data
    self.prev = prev or self # prev가 None이면 자기 자신을 참조
    self.next = next or self # next가 None이면 자기 자신을 참조
...
class DoubleLinkedList:
  ...
  def search(self, data: Any) -> Any:
    """data와 값이 같은 노드를 검색"""
    cnt = 0
    ptr = self.head.next # 더미 노드 다음부터 검색
  ...
  def prev(self) -> bool:
    """주목 노드를 한 칸 앞으로 이동"""
    if self.is_empty() or self.head.prev == self.head:
      return False
  ...
  def remove_current_node(self) -> None:
    ...
    if not self.is_empty():
      ...
      self.current = self.current.prev
      self.no -= 1
      if self.current is self.head: # 더미 노드라면
        self.current = self.current.next
